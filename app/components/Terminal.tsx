...
export function Terminal() {
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (e.key === "Tab") {
        e.preventDefault();
        const result = getAutocompletions(
        ...
        if (result.completions.length === 1) {
          ...
          result.commonPrefix.length >
            terminalState.currentInput.split(/\s+/).pop()?.length!
        ) {
          ...
          const newOutputLines = [...outputLines];
          const prompt = `${formatPath(
            terminalState.filesystem.currentPath
          )} $ `;
  ...
  const currentPrompt = `${formatPath(
    terminalState.filesystem.currentPath
  )} $ `;
          ...
          <div className="flex items-center font-mono text-sm">
            ...
            <div className="flex-1 relative">
              <input
                ...
                className="w-full bg-transparent text-ctp-text outline-none border-none caret-ctp-text"
                spellCheck={false}
                autoComplete="off"
              />
            </div>
